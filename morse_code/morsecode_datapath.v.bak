...
module test(enable,load, reset,q_out, shiftreg_out,letter_in,testlight);
	//mong muon:
	...
	assign testlight = shiftreg_out[0];
	
	morsecode_lengthcounter U0(.enable(enable),.load(load),.data_in(morsecode_length),.q_out(q_out),.reset(reset));
